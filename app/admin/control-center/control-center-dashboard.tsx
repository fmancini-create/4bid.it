...
export default function ControlCenterDashboard({ userEmail, latestRuns, findings, history, storageReady, githubReady, githubFixReady }: { userEmail: string; latestRuns: Latest[]; findings: Row[]; history: Row[]; storageReady: boolean; githubReady: boolean; githubFixReady: boolean }) {
  ...
  async function prepareManual(item: Row) {
    setPreparing(item.id)
    setPendingPr(null)
    setManualPlan(null)
    setMessage(null)
    try {
      const response = await fetch("/api/admin/control-center/prepare", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ findingId: item.id }),
      })
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || "Preparazione intervento non riuscita")
      setManualPlan(payload.plan)
      setMessage("Piano di intervento preparato. Nessuna modifica e' stata applicata al repository.")
      requestAnimationFrame(() => document.getElementById("manual-intervention-plan")?.scrollIntoView({ behavior: "smooth", block: "start" }))
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Preparazione intervento non riuscita")
    } finally {
      setPreparing(null)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 space-y-6">
        ...
        {manualPlan && <Card id="manual-intervention-plan" className="border-primary/30"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Wrench className="h-5 w-5" />Piano di intervento</CardTitle><CardDescription>{manualPlan.repository} · {manualPlan.branch}</CardDescription></div><div className="flex gap-2"><Badge variant="outline">{manualPlan.changeType}</Badge><Badge variant={manualPlan.risk === "alto" ? "destructive" : "secondary"}>Rischio {manualPlan.risk}</Badge></div></div></CardHeader><CardContent className="space-y-5"><div><p className="font-semibold">{manualPlan.title}</p><p className="text-sm text-muted-foreground mt-1">{manualPlan.summary}</p></div>{manualPlan.files.length > 0 && <div><p className="text-sm font-semibold mb-2">File/aree da verificare</p><div className="flex flex-wrap gap-2">{manualPlan.files.map((file) => <Badge key={file} variant="outline" className="font-mono text-xs">{file}</Badge>)}</div></div>}<div><p className="text-sm font-semibold mb-2">Intervento proposto</p><ol className="list-decimal pl-5 space-y-1.5 text-sm">{manualPlan.steps.map((step) => <li key={step}>{step}</li>)}</ol></div><div><p className="text-sm font-semibold mb-2">Controlli prima del merge</p><ul className="list-disc pl-5 space-y-1.5 text-sm">{manualPlan.checks.map((check) => <li key={check}>{check}</li>)}</ul></div><div className="flex flex-wrap gap-2"><Button variant="outline" asChild><a href={manualPlan.repositoryUrl} target="_blank" rel="noreferrer">Apri repository<ExternalLink className="h-3.5 w-3.5 ml-1" /></a></Button><Button variant="ghost" onClick={() => setManualPlan(null)}>Chiudi piano</Button></div><p className="text-xs text-muted-foreground">Questo pulsante prepara l'intervento: non modifica codice, non crea branch e non effettua merge finche' il finding non e' classificato come auto-correggibile.</p></CardContent></Card>}
