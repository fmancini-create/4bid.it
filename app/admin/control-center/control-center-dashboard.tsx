    ...
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3"><Activity className="h-7 w-7 text-primary" /><div><h1 className="text-2xl font-bold">4 BID Control Center</h1><p className="text-sm text-muted-foreground">Salute tecnica dell'ecosistema · {userEmail}</p></div></div>
          <div className="flex gap-2"><Button variant="outline" asChild><a href="/admin"><ArrowLeft className="h-4 w-4 mr-2" />Admin</a></Button><Button onClick={() => runAudit()} disabled={Boolean(running) || !storageReady || !githubReady}><Play className="h-4 w-4 mr-2" />{running === "all" ? "Analisi in corso…" : "Analizza tutto"}</Button></div>
        </div>

        {(!storageReady || !githubReady || message) && <div className={`rounded-lg border p-4 text-sm ${!storageReady || !githubReady ? "border-amber-300 bg-amber-50 text-amber-900" : "bg-card"}`}>{!storageReady && <p><strong>Database da inizializzare.</strong> Applicare `scripts/071_technical_audit_control_center.sql`.</p>}{!githubReady && <p><strong>GitHub non collegato.</strong> Configurare `GITHUB_AUDIT_TOKEN` server-side con accesso in lettura ai cinque repository.</p>}{message && <p>{message}</p>}</div>}

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><Card><CardHeader className="pb-2"><CardDescription>Prodotti</CardDescription><CardTitle>{latestRuns.length}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Punteggio medio</CardDescription><CardTitle className={scoreColor(averageScore ?? undefined)}>{averageScore ?? "—"}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Critici</CardDescription><CardTitle className="text-red-600">{critical}</CardTitle></CardHeader></Card><Card><CardHeader className="pb-2"><CardDescription>Da attenzionare</CardDescription><CardTitle className="text-amber-600">{attention}</CardTitle></CardHeader></Card></div>
