...
import { Activity, AlertTriangle, ArrowLeft, CheckCircle2, Clock3, ExternalLink, GitBranch, Loader2, Play, Wrench, XCircle } from "lucide-react"
  ...
  async function remediate(input: { findingId?: string; project?: string; allFixable?: boolean }, key: string) {
    ...
    try {
      ...
      if (payload.requiresReview) {
        ...
        setMessage(`Correzione preparata in sicurezza su branch ${payload.branch}. Creata PR #${payload.prNumber}; verifica CI e diff prima del merge.`)
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8 space-y-6">
        ...
        {(!storageReady || !githubReady || !githubFixReady || message) && <div className={`rounded-lg border p-4 text-sm space-y-2 ${!storageReady || !githubReady || !githubFixReady ? "border-amber-300 bg-amber-50 text-amber-900" : "bg-card"}`}>{!storageReady && <p><strong>Database da inizializzare.</strong> Applicare `scripts/071_technical_audit_control_center.sql`.</p>}{!githubReady && <p><strong>GitHub non collegato.</strong> Configurare `GITHUB_AUDIT_TOKEN` server-side con accesso in sola lettura a tutti i repository dell'account `fmancini-create`.</p>}{!githubFixReady && <p><strong>Interventi automatici non ancora abilitati.</strong> Configurare `GITHUB_FIX_TOKEN` separato dal token di audit. Il token di audit resta rigorosamente read-only.</p>}{message && <p>{message}</p>}{prUrl && <p><a className="inline-flex items-center gap-1 font-semibold underline" href={prUrl} target="_blank" rel="noreferrer">Apri la Pull Request <ExternalLink className="h-3.5 w-3.5" /></a></p>}</div>}
