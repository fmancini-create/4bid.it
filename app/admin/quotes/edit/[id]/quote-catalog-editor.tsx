...
export default function QuoteCatalogEditor({ quoteId }: { quoteId: string }) {
  ...
  async function persistQuotePatch(patch: Partial<SalesChannelQuote>) {
    try {
      const response = await fetch(`/api/quotes/${quoteId}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(patch),
        // Il salvataggio deve completarsi anche se subito dopo l'utente preme
        // "Salva struttura", che ricarica la pagina.
        keepalive: true,
      })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(data.error || "Salvataggio automatico fallito")
    } catch (error: any) {
      toast.error(`Dati cliente non salvati: ${error?.message || "errore sconosciuto"}`)
    }
  }

  function applyCompany(d: CompanyLookupData) {
    const patch: Partial<SalesChannelQuote> = {
  ...
  return <div className="max-w-7xl mx-auto space-y-7 pb-20">
    ...
    <section className="border rounded-xl p-5 bg-card space-y-4"><div><h2 className="font-semibold text-lg">Cliente e proposta</h2><p className="text-xs text-muted-foreground">I dati di questa sezione si salvano automaticamente quando esci dal campo, così non vengono persi usando gli altri comandi di salvataggio della pagina.</p></div><div className="grid md:grid-cols-2 gap-4"><div><Label>Referente</Label><Input value={quote.client_name || ""} onChange={e => patchQuote({ client_name: e.target.value })} onBlur={() => void persistQuotePatch({ client_name: quote.client_name?.trim() || null })} /></div><div><Label>Azienda</Label><Input value={quote.client_company || ""} onChange={e => patchQuote({ client_company: e.target.value })} onBlur={() => void persistQuotePatch({ client_company: quote.client_company?.trim() || null })} /></div><div><Label>Email</Label><Input type="email" value={quote.client_email || ""} onChange={e => patchQuote({ client_email: e.target.value })} onBlur={() => { const email = quote.client_email?.trim() || null; patchQuote({ client_email: email }); void persistQuotePatch({ client_email: email }) }} /></div><CompanyLookupField value={quote.client_vat || ""} onValueChange={v => patchQuote({ client_vat: v })} onApply={applyCompany} /><div className="md:col-span-2"><Label>Indirizzo</Label><Input value={quote.client_address || ""} onChange={e => patchQuote({ client_address: e.target.value })} onBlur={() => void persistQuotePatch({ client_address: quote.client_address?.trim() || null })} /></div><div className="md:col-span-2"><Label>Titolo</Label><Input value={quote.title || ""} onChange={e => patchQuote({ title: e.target.value })} onBlur={() => void persistQuotePatch({ title: quote.title?.trim() || null })} /></div><div className="md:col-span-2"><Label>Testo / descrizione</Label><Textarea rows={4} value={quote.description || ""} onChange={e => patchQuote({ description: e.target.value })} onBlur={() => void persistQuotePatch({ description: quote.description || null })} /></div><div className="md:col-span-2"><Label>Condizioni</Label><Textarea rows={3} value={quote.payment_terms || ""} onChange={e => patchQuote({ payment_terms: e.target.value })} onBlur={() => void persistQuotePatch({ payment_terms: quote.payment_terms || null })} /></div><div><Label>Offerta valida fino al</Label><Input type="datetime-local" value={toLocalDateTime(quote.expires_at)} onChange={e => patchQuote({ expires_at: e.target.value ? new Date(e.target.value).toISOString() : null })} onBlur={() => void persistQuotePatch({ expires_at: quote.expires_at || null })} /></div><div className="flex items-center gap-2 mt-6"><Switch checked={quote.vat_included ?? true} onCheckedChange={v => { patchQuote({ vat_included: v }); void persistQuotePatch({ vat_included: v }) }} /><Label>IVA inclusa</Label></div></div></section>
    ...
    <section className="space-y-4"><h2 className="font-semibold text-xl">Voci del preventivo</h2>{calculated.map((item, index) => {
      ...
      >
        ...
        {(item.kind === "setup" || item.kind === "service") && item.billing_period === "one_time" ? <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3"><div className="flex items-center justify-between gap-3"><div><strong>{item.kind === "service" ? "Agevolazione servizio con piano annuale" : "Agevolazione setup con piano annuale"}</strong><p className="text-xs text-muted-foreground">Scegli se questa voce una tantum resta a prezzo pieno, viene scontata o azzerata quando il cliente seleziona la quota annuale.</p></div><Switch checked={setupAnnualMode !== "full"} onCheckedChange={enabled => patchSetupAnnualPolicy(index, enabled ? "free" : "full")} /></div>{setupAnnualMode !== "full" ? <div className="grid sm:grid-cols-2 gap-3"><div><Label>Trattamento con annuale</Label><Select value={setupAnnualMode} onValueChange={value => patchSetupAnnualPolicy(index, value as AnnualSetupMode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="discount">Scontato</SelectItem><SelectItem value="free">Azzerato / omaggio</SelectItem></SelectContent></Select></div>{setupAnnualMode === "discount" ? <div><Label>{item.kind === "service" ? "Sconto servizio %" : "Sconto setup %"}</Label><Input type="number" min="0" max="100" step="0.1" value={meta.annual_setup_discount_pct || 0} onChange={e => patchSetupAnnualPolicy(index, "discount", Number(e.target.value))} /></div> : <div className="flex items-end"><p className="rounded-md bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-800">{item.kind === "service" ? "Servizio azzerato con formula annuale" : "Setup azzerato con formula annuale"}</p></div>}</div> : <p className="text-xs text-muted-foreground">Disattivato: la voce una tantum viene addebitata per intero anche con formula annuale.</p>}</div> : null}
        {item.billing_period === "monthly" ? (() => {
          ...
          const annualFull = Math.round(monthly * quantity * 12 * 100) / 100
          ...
          return <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Sconto applicato quando il cliente sceglie di pagare subito l&apos;intero anno invece del canone mensile (base annuale {formatQuoteAmount(annualFull, quote.currency)} = canone × quantità × 12 mesi).</p>
              </div>
