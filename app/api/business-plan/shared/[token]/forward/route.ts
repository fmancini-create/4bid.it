...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const planName = sourceShare.business_plans?.client_name || sourceShare.business_plans?.name || "Preventivo"
  ...
  for (const email of emails) {
    ...
    if (insertError || !recipientShare) {
      console.error("[forward] share creation failed", email, insertError)
    ...
    const html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#222;background:#f5f5f5;padding:24px"><div style="max-width:600px;margin:auto;background:#fff;padding:32px;border-radius:10px"><h2>${planName}</h2><p>Ti è stato inoltrato un preventivo/documento 4BID.</p><p><strong>Password di accesso:</strong> <span style="font-family:monospace;font-size:18px">${password}</span></p><p style="margin:28px 0"><a href="${link}" style="background:#f59e0b;color:#fff;padding:13px 22px;border-radius:6px;text-decoration:none;font-weight:bold">Visualizza preventivo</a></p><p style="font-size:12px;color:#777">Il link è personale e permette di registrare le visualizzazioni del documento.</p><img src="${pixel}" width="1" height="1" alt="" style="display:block;border:0" /></div></body></html>`

    const emailResult = await sendEmail({ to: email, subject: `${planName} - Preventivo condiviso`, html })
    const success = !!emailResult.success
    results.push({ email, success })

    if (success) {
      await supabase.from("business_plan_share_events").insert([
        { share_id: recipientShare.id, business_plan_id: sourceShare.business_plan_id, event_type: "forwarded", recipient_email: email, metadata: { forwarded_by_share_id: sourceShare.id } },
