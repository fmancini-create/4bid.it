...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (current.status === "paid") {
    return NextResponse.json({ error: "Un preventivo pagato è congelato e non può essere modificato" }, { status: 409 })
  }
  ...
  if (Array.isArray(body.line_items)) {
    const lines = body.line_items.map((item: QuoteLineItem) => calculateQuoteLine({
      ...item,
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (current?.status === "paid") {
    return NextResponse.json({ error: "Un preventivo pagato non può essere eliminato" }, { status: 409 })
  }
