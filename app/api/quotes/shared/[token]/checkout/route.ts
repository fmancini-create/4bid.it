...
export async function POST(request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (quote.stripe_session_id) {
    try {
      ...
      if (previousSession.status === "complete") {
        return NextResponse.json({ error: "Il pagamento è già stato completato ed è in elaborazione. Non avviare un secondo checkout.", code: "CHECKOUT_PROCESSING" }, { status: 409 })
      }
    } catch (cause: any) {
  ...
  if (!hasRecurring && !oneTimePayableItems.length) return NextResponse.json({ error: "Il preventivo non contiene importi da addebitare con carta" }, { status: 400 })
  ...
  if (requiresOrchestratedSetup(items)) {
    const session = await stripe.checkout.sessions.create({
      mode: "setup", customer_creation: "always", payment_method_types: ["card"], customer_email: quote.client_email || undefined,
      ...
      success_url: `${baseUrl}/preventivo/${token}?paid=processing`, cancel_url: `${baseUrl}/preventivo/${token}`,
      expires_at: Math.floor(Date.now() / 1000) + 30 * 60, billing_address_collection: "required", tax_id_collection: { enabled: true },
    }, { idempotencyKey: checkoutIdempotencyKey })
    await supabase.from("sales_channel_quotes").update({ stripe_session_id: session.id, payment_method: "card", updated_at: new Date().toISOString() }).eq("id", quote.id)
  ...
  if (tempDiscountItems.length) {
    ...
    const coupon = await stripe.coupons.create({
      ...
      ...(type === "percentage" ? { percent_off: Math.min(100, Math.max(0, value)) } : { amount_off: Math.max(0, Math.round(value * 100)), currency }),
      metadata: { type: "sales_channel_quote", quote_id: quote.id }, name: stripeCouponName(`${quote.quote_number || "Preventivo"} - sconto ${durationMonths}m`),
  ...
  const checkoutItems = items.filter(item => (item.billing_period && item.billing_period !== "one_time") || item.amount > 0)
  const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = checkoutItems.map(item => {
    ...
    return { quantity: 1, price_data: { currency, unit_amount: Math.max(0, Math.round(stripeAmount * 100)), product_data: { name: item.name || item.description, description: item.features?.length ? item.features.slice(0, 5).join(" · ").slice(0, 500) : item.description.slice(0, 500), metadata: { quote_id: quote.id, project: item.project || "custom", source_product_id: item.source_product_id || "" } }, ...(recurring ? { recurring } : {}) } }
  })

  const trialDays = hasRecurring ? Math.max(0, Number(recurringItems[0]?.trial_days) || 0) : 0
  const common: Stripe.Checkout.SessionCreateParams = {
    payment_method_types: ["card"], customer_email: quote.client_email || undefined, line_items: lineItems,
    metadata: { type: "sales_channel_quote", quote_id: quote.id, ...(temporaryDiscountCouponId ? { temporary_discount_coupon_id: temporaryDiscountCouponId } : {}) },
    success_url: `${baseUrl}/preventivo/${token}?paid=1`, cancel_url: `${baseUrl}/preventivo/${token}`,
    expires_at: Math.floor(Date.now() / 1000) + 30 * 60, billing_address_collection: "required", tax_id_collection: { enabled: true }, allow_promotion_codes: false,
    ...(checkoutDiscount ? { discounts: [checkoutDiscount] } : {}),
  }

  const session = hasRecurring
    ? await stripe.checkout.sessions.create({ ...common, mode: "subscription", payment_method_collection: "always", subscription_data: { metadata: { type: "sales_channel_quote", quote_id: quote.id }, ...(trialDays > 0 ? { trial_period_days: trialDays } : {}) } }, { idempotencyKey: checkoutIdempotencyKey })
    : await stripe.checkout.sessions.create({ ...common, mode: "payment", payment_intent_data: { metadata: { type: "sales_channel_quote", quote_id: quote.id } } }, { idempotencyKey: checkoutIdempotencyKey })
