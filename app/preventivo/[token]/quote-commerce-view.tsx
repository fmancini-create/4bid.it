...
function LineItemCard({ item, currency }: { item: QuoteLineItem; currency: string }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-background p-5 space-y-4">
      ...
      {(item.quantity || 1) > 1 ? <p className="text-xs text-muted-foreground">Quantità: <strong className="text-foreground">{item.quantity}</strong>{item.unit_amount != null ? ` × ${formatQuoteAmount(item.unit_amount, currency)}` : ""}</p> : null}
...
export default function QuoteCommerceView({ token, quote, expired }: Props) {
  ...
  const requiresCard = useMemo(() => lineItems.some(item => SAAS_PROJECTS.has(item.project || "") || Boolean(item.billing_period && item.billing_period !== "one_time")), [lineItems])
  ...
  const [billing, setBilling] = useState<QuoteBillingDetails>(() => {
    ...
    return { company: saved.company || quote.client_company || "", vat: saved.vat || quote.client_vat || "", tax_code: saved.tax_code || "", address: saved.address || quote.client_address || "", zip: saved.zip || "", city: saved.city || "", province: saved.province || "", sdi_code: saved.sdi_code || "", pec: saved.pec || "", reference: saved.reference || quote.client_name || "" }
  })
  ...
  async function acceptQuote() {
    ...
    if (requiresCard && paymentMethod !== "card") return toast.error("Per questa offerta è richiesta una carta di credito")
    ...
    setAccepting(true)
    try {
      const response = await fetch(`/api/quotes/shared/${token}/accept`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ submitted_fields: fieldValues, billing_details: billing, acceptance_name: acceptanceName.trim(), accepted: true, payment_method: paymentMethod }) })
      const data = await response.json().catch(() => ({}))
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
        ...
        <section className="space-y-3"><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" /><h2 className="text-xl font-semibold">Soluzioni incluse</h2></div>{lineItems.map((item, index) => <LineItemCard key={item.id || index} item={item} currency={currency} />)}</section>

        <section className="rounded-2xl border border-border bg-card p-6"><h2 className="mb-4 text-lg font-semibold">Riepilogo economico</h2><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{totals.oneTime > 0 ? <div className="rounded-xl bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Una tantum</p><p className="text-xl font-bold">{formatQuoteAmount(totals.oneTime, currency)}</p></div> : null}{totals.monthly > 0 ? <div className="rounded-xl bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Mensile</p><p className="text-xl font-bold">{formatQuoteAmount(totals.monthly, currency)}</p></div> : null}{totals.quarterly > 0 ? <div className="rounded-xl bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Trimestrale</p><p className="text-xl font-bold">{formatQuoteAmount(totals.quarterly, currency)}</p></div> : null}{totals.yearly > 0 ? <div className="rounded-xl bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Annuale</p><p className="text-xl font-bold">{formatQuoteAmount(totals.yearly, currency)}</p></div> : null}</div>{totals.savings > 0 ? <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900"><strong>Vantaggio economico evidenziato:</strong> {formatQuoteAmount(totals.savings, currency)} rispetto al listino configurato.</div> : null}<p className="mt-3 text-right text-xs text-muted-foreground">{quote.vat_included ? "Importi IVA inclusa" : "Importi IVA esclusa"}</p></section>
        ...
        {!alreadyAccepted && !expired ? <>
          ...
          <section className="rounded-2xl border-2 border-primary/20 bg-card p-6 space-y-5"><div><h2 className="text-xl font-semibold">Accetta il preventivo</h2><p className="text-sm text-muted-foreground">L'accettazione congela condizioni, prezzi, funzionalità e servizi descritti in questa proposta.</p></div><div><Label>Nome e cognome dell'accettante *</Label><Input value={acceptanceName} onChange={e => setAcceptanceName(e.target.value)} /></div>
            {requiresCard ? <div className="rounded-xl border border-primary bg-primary/5 p-4"><div className="flex items-center gap-2"><CreditCard className="h-5 w-5" /><strong>Carta di credito richiesta</strong></div><p className="mt-1 text-xs text-muted-foreground">Necessaria per attivazione automatica, eventuale periodo di prova e rinnovi dei servizi SaaS.</p></div> : <div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setPaymentMethod("card")} className={`rounded-xl border p-4 text-left transition ${paymentMethod === "card" ? "border-primary bg-primary/5" : "border-border"}`}><CreditCard className="mb-2 h-5 w-5" /><strong className="block">Carta di credito</strong><span className="text-xs text-muted-foreground">Pagamento sicuro e attivazione automatica</span></button><button type="button" onClick={() => setPaymentMethod("bonifico")} className={`rounded-xl border p-4 text-left transition ${paymentMethod === "bonifico" ? "border-primary bg-primary/5" : "border-border"}`}><Banknote className="mb-2 h-5 w-5" /><strong className="block">Bonifico bancario</strong><span className="text-xs text-muted-foreground">Attivazione dopo registrazione dell'incasso</span></button></div>}
            <label className="flex items-start gap-3 text-sm"><Checkbox checked={acceptedTerms} onCheckedChange={value => setAcceptedTerms(value === true)} /><span>Confermo di aver letto e accettato integralmente il preventivo, le condizioni economiche, le funzionalità incluse, eventuali periodi di prova e le condizioni di assistenza indicate.</span></label><Button size="lg" className="w-full" onClick={acceptQuote} disabled={accepting || paying}>{accepting || paying ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />}{paymentMethod === "card" ? "Accetta e vai al pagamento" : "Accetta preventivo"}</Button></section>
        </> : null}
