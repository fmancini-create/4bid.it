...
const projectNames: Record<string, string> = { hotelaccelerator: "HotelAccelerator", santaddeo: "Santaddeo", hotelprofitai: "HotelProfitAI", manubot: "ManuBot", consulting: "Consulenza 4BID", custom: "Servizio personalizzato" }
...
function LineItemCard({ item, currency, selected, locked, parentSelected, onSelectedChange }: { item: QuoteLineItem; currency: string; selected: boolean; locked: boolean; parentSelected: boolean; onSelectedChange: (value: boolean) => void }) {
  ...
  const isFreeAnnualService = calculated.billing_period === "one_time" && meta.free_on_annual && Number(calculated.amount || 0) === 0
  return <article className={`rounded-xl border bg-background p-5 space-y-4 transition ${selected && parentSelected ? "border-primary/40" : "border-border opacity-60"}`}>
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3 min-w-0">{item.optional ? <Checkbox checked={selected && parentSelected} disabled={locked || !parentSelected} onCheckedChange={value => onSelectedChange(value === true)} className="mt-1" /> : <div className="mt-1 flex h-4 w-4 items-center justify-center rounded-sm bg-primary text-primary-foreground"><Check className="h-3 w-3" /></div>}<div className="min-w-0"><div className="mb-1 flex flex-wrap items-center gap-2"><span className="text-xs font-semibold uppercase tracking-wide text-primary">{projectNames[item.project || "custom"] || item.project || "4BID"}</span><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${item.optional ? "bg-amber-100 text-amber-800" : "bg-muted text-muted-foreground"}`}>{item.optional ? "Opzionale" : "Inclusa"}</span>{item.trial_days ? <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">{item.trial_days} giorni di prova</span> : null}{isFreeAnnualService ? <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-800">OMAGGIO CON ANNUALE</span> : null}</div><h3 className="text-lg font-semibold">{item.name || item.description}</h3>{item.name && item.description && item.description !== item.name ? <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{item.description}</p> : null}</div></div><div className="shrink-0 text-left sm:text-right">{hasDiscount || isFreeAnnualService ? <div className="text-sm text-muted-foreground line-through">{formatQuoteAmount(listAmount || Number(meta.normal_price || 0), currency)}</div> : null}<div className={`text-xl font-bold ${isFreeAnnualService ? "text-emerald-700" : ""}`}>{isFreeAnnualService ? "OMAGGIO" : formatQuoteAmount(calculated.amount, currency)}</div><div className="text-xs text-muted-foreground">{calculated.billing_period === "one_time" ? "una tantum" : `/${period}`}</div>{hasDiscount ? <div className="mt-1 text-xs font-semibold text-emerald-700">Risparmio {formatQuoteAmount(discountAmount, currency)}{item.discount?.type === "percentage" ? ` (${item.discount.value}%)` : ""}</div> : null}</div></div>
...
export default function QuoteCommerceView({ token, quote, expired }: Props) {
  ...
  const totals = useMemo(() => ({
    oneTime: selectedItems.filter(i=>i.billing_period==="one_time").reduce((s,i)=>s+Number(i.amount||0),0),
    recurring: selectedItems.filter(i=>i.billing_period===billingPreference).reduce((s,i)=>s+Number(i.amount||0),0),
    savings: selectedItems.reduce((s,i)=>s+Number(i.discount_amount||0),0),
  }), [selectedItems,billingPreference])
  ...
  async function acceptQuote(){
    ...
    if(billingPreference==="yearly"&&!annualEligible)return toast.error("La formula annuale non è disponibile per tutte le voci selezionate")
  ...
  return <div className="min-h-screen bg-muted/30"><header className="border-b bg-background"><div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-5 sm:px-6"><Image src="/logo.png" alt="4BID" width={110} height={44} className="h-10 w-auto" priority/><div className="text-right"><p className="text-sm font-semibold">Proposta commerciale</p>{quote.quote_number?<p className="text-xs text-muted-foreground">N. {quote.quote_number}</p>:null}</div></div></header>
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      ...
      {!alreadyAccepted && recurringParents.length>0?<section className="rounded-2xl border-2 border-primary/20 bg-card p-6"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-2"><Clock3 className="h-5 w-5 text-primary"/><h2 className="text-xl font-semibold">Scegli la formula di abbonamento</h2></div><p className="mt-1 text-sm text-muted-foreground">La tariffa mensile è mostrata di default. Passa all'annuale per vedere il risparmio riservato.</p></div><div className="inline-flex rounded-xl border bg-muted p-1"><button type="button" onClick={()=>setBillingPreference("monthly")} className={`rounded-lg px-5 py-3 text-sm font-semibold ${billingPreference==="monthly"?"bg-background shadow":""}`}>Mensile</button><button type="button" disabled={!annualEligible} onClick={()=>annualEligible&&setBillingPreference("yearly")} className={`rounded-lg px-5 py-3 text-sm font-semibold ${billingPreference==="yearly"?"bg-emerald-600 text-white shadow":""} ${!annualEligible?"opacity-40 cursor-not-allowed":""}`}>Annuale{annualBenefit.amount>0?` · -${annualBenefit.pct}%`:""}</button></div></div>{annualEligible&&annualBenefit.amount>0?<div className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm font-medium text-emerald-800">Con la formula annuale risparmi {formatQuoteAmount(annualBenefit.amount,currency)} rispetto a 12 mensilità.</div>:!annualEligible?<p className="mt-3 text-xs text-muted-foreground">L'annuale non è disponibile per almeno una delle voci ricorrenti selezionate.</p>:null}</section>:null}

      <section className="space-y-3"><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary"/><div><h2 className="text-xl font-semibold">Configura la tua proposta</h2>{rawItems.some(i=>i.optional)&&!alreadyAccepted?<p className="text-sm text-muted-foreground">Puoi selezionare o deselezionare le voci opzionali.</p>:null}</div></div>{effectiveItems.map((item,index)=><LineItemCard key={item.id||index} item={item} currency={currency} selected={!item.optional||!!item.id&&selectedIds.has(item.id)} parentSelected={parentSelected(item)} locked={alreadyAccepted} onSelectedChange={value=>setItemSelected(item,value)}/>)}</section>

      <section className="sticky bottom-3 z-10 rounded-2xl border border-primary/20 bg-background/95 p-6 shadow-lg backdrop-blur"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="mb-3 text-lg font-semibold">Riepilogo</h2><div className="flex flex-wrap gap-8">{totals.oneTime>0?<div><p className="text-xs text-muted-foreground">Una tantum</p><p className="text-xl font-bold">{formatQuoteAmount(totals.oneTime,currency)}</p></div>:null}<div><p className="text-xs text-muted-foreground">{billingPreference==="yearly"?"Canone annuale":"Canone mensile"}</p><p className="text-2xl font-black">{formatQuoteAmount(totals.recurring,currency)}</p></div></div></div><p className="text-xs text-muted-foreground">{quote.vat_included?"Importi IVA inclusa":"Importi IVA esclusa"}</p></div></section>

      {quote.payment_terms?<section className="rounded-2xl border bg-card p-6"><h2 className="mb-2 font-semibold">Durata, rinnovo e condizioni</h2><p className="whitespace-pre-wrap text-sm leading-relaxed text-muted-foreground">{quote.payment_terms}</p></section>:null}
      ...
      {!alreadyAccepted&&!expired?<>
        ...
        <section className="rounded-2xl border-2 border-primary/20 bg-card p-6 space-y-5"><div><h2 className="text-xl font-semibold">Accetta la configurazione selezionata</h2><p className="text-sm text-muted-foreground">Con l'accettazione vengono congelati prodotti, periodicità, prezzi, omaggi e condizioni.</p></div><div><Label>Nome e cognome dell'accettante *</Label><Input value={acceptanceName} onChange={e=>setAcceptanceName(e.target.value)}/></div>{requiresCard?<div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><CreditCard className="mb-2 h-5 w-5"/><strong className="block">Carta di credito richiesta</strong><span className="text-xs text-muted-foreground">Necessaria per attivazione e rinnovi automatici.</span></div>:<div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={()=>setPaymentMethod("card")} className={`rounded-xl border p-4 text-left ${paymentMethod==="card"?"border-primary bg-primary/5":""}`}><CreditCard className="mb-2 h-5 w-5"/><strong>Carta</strong></button><button type="button" onClick={()=>setPaymentMethod("bonifico")} className={`rounded-xl border p-4 text-left ${paymentMethod==="bonifico"?"border-primary bg-primary/5":""}`}><Banknote className="mb-2 h-5 w-5"/><strong>Bonifico</strong></button></div>}<label className="flex items-start gap-3 text-sm"><Checkbox checked={acceptedTerms} onCheckedChange={v=>setAcceptedTerms(v===true)}/><span>Confermo di aver letto e accettato il preventivo, la durata selezionata, il rinnovo automatico e le relative condizioni economiche e operative.</span></label><Button size="lg" className="w-full" onClick={acceptQuote} disabled={accepting||paying}>{accepting||paying?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<CheckCircle2 className="mr-2 h-4 w-4"/>}{requiresCard||paymentMethod==="card"?"Accetta e vai al pagamento":"Accetta preventivo"}</Button></section>
      </>:null}
